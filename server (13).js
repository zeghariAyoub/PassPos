...
const MIXPANEL_TOKEN = '4d98e9edb93627bee96322e164d09655'
const mpTrack = async (event, props = {}) => {
  try {
    const data = { event, properties: { token: MIXPANEL_TOKEN, time: Math.floor(Date.now()/1000), platform: 'passpos_backend', ...props } }
    await fetch('https://api.mixpanel.com/track', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify([data])
    })
  } catch(e) { console.log('Mixpanel error:', e.message) }
}
...
${lesson.content.slice(0, 4000)}`
  ...
  try {
    ...
    console.log('FIRST QUESTION KEYS:', questions[0] ? Object.keys(questions[0]) : 'none')
    console.log('FIRST QUESTION:', JSON.stringify(questions[0]).slice(0, 300))
  ...
  const sanitized = questions.map(q => ({
    question: String(q.question || q.question_text || q.text || q.stem || ''),
    option_a: String(q.option_a || q.optionA || q.options?.A || q.choices?.A || q.a || ''),
    option_b: String(q.option_b || q.optionB || q.options?.B || q.choices?.B || q.b || ''),
    option_c: String(q.option_c || q.optionC || q.options?.C || q.choices?.C || q.c || ''),
    option_d: String(q.option_d || q.optionD || q.options?.D || q.choices?.D || q.d || ''),
    correct_answer: String(q.correct_answer || q.correctAnswer || q.answer || q.correct || 'A').toUpperCase().charAt(0),
    explanation: String(q.explanation || q.rationale || q.reason || ''),
    topic: String(q.topic || q.subject || q.category || ''),
    difficulty: ['easy','medium','hard'].includes(q.difficulty) ? q.difficulty : 'medium'
  }))
  console.log('SANITIZED[0]:', JSON.stringify(sanitized[0]))
  const filtered = sanitized.filter(q => q.question && q.option_a && q.option_b)
  console.log('FILTERED COUNT:', filtered.length, 'of', sanitized.length)
  const sanitizedFinal = filtered

  if (sanitizedFinal.length === 0) return { error: 'No valid questions generated. Gemini returned: ' + JSON.stringify(questions[0] || {}).slice(0,200) }
  ...
  const toInsert = sanitizedFinal.map(q => ({ ...q, quiz_id: quiz.id }))
  ...
  mpTrack('quiz_generated', { quiz_id: quiz.id, lesson_id: lessonId, title, questions_created: sanitized.length, mode, hints_allowed: hintsAllowed })
  return { quiz_id: quiz.id, title: quiz.title, questions_created: sanitizedFinal.length, mode, hints_allowed: hintsAllowed }
}
...
app.post('/api/agent', async (req, res) => {
  ...
  try {
    ...
    // GENERATE QUIZ OR EXAM — must come FIRST before list-lessons catches it
    if (msg.includes('generate') || (msg.includes('create') && (msg.includes('quiz') || msg.includes('exam')))) {
...
' +
            '👉 Go to the 📝 Quizzes tab and click 🚀 Publish!'
        }
      }
    }

    // LIST LESSONS
    else if ((msg.includes('list') || msg.includes('show') || msg.includes('my')) && msg.includes('lesson')) {
    ...
    // PUBLISH QUIZ
      ...
      if (!quizId) {
        ...
        if (!updated) {
          ...
          mpTrack('quiz_published', { quiz_id: quizId, title: updated.title })
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    mpTrack('lesson_uploaded', { lesson_id: lesson.id, title, subject, chunks: count })
