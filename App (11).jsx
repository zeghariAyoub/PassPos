...
function TeacherAgent() {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      const { reply } = await api.agent({ message: msg, history })

      let finalReply = reply || 'I could not process that request. Please try again.'

      if (false && toolResults) {
        const parts = []
...
function QuizzesTab() {
  ...
  const load = () => {
    setLoading(true)
    fetch(`${BACKEND_URL}/api/quizzes/all`).then(r => r.json()).then(d => {
  ...
  const publish = async (id) => {
    setPublishing(id)
    await fetch(`${BACKEND_URL}/api/agent`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: `Publish quiz ${id}`, history: [] })
    })
    await load()
    setPublishing(null)
  }
  ...
  const QuizCard = ({ q, showPublish }) => (
    <div style={{ background: '#151820', border: `1px solid ${showPublish ? '#F7C94840' : '#3DD68C30'}`, borderRadius: 12, padding: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <div>
        ...
        <div style={{ color: '#7A8099', fontSize: 11 }}>ID: {q.id.slice(0, 8)}... · {new Date(q.created_at).toLocaleDateString()}</div>
      </div>
      {showPublish
        ? <button onClick={() => publish(q.id)} disabled={publishing === q.id}
            style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: publishing === q.id ? '#252A3A' : '#3DD68C', color: publishing === q.id ? '#7A8099' : '#0F1117', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ background: '#1C2030', border: '1px solid #252A3A', borderRadius: 16, padding: 28 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          <h2 style={{ fontFamily: 'Lora, serif', fontSize: 18, margin: 0 }}>🔴 Drafts ({drafts.length})</h2>
          <button onClick={load} style={{ background: 'none', border: '1px solid #252A3A', color: '#7A8099', borderRadius: 8, padding: '4px 12px', fontSize: 12, cursor: 'pointer' }}>🔄 Refresh</button>
        </div>
        <p style={{ color: '#7A8099', fontSize: 13, marginBottom: 20 }}>Generated but not visible to students yet.</p>
        {loading ? <p style={{ color: '#7A8099', fontSize: 13 }}>Loading...</p>
          : drafts.length === 0 ? <p style={{ color: '#7A8099', fontSize: 13 }}>No drafts — generate a quiz from the AI Assistant!</p>
          : <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>{drafts.map(q => <QuizCard key={q.id} q={q} showPublish={true} />)}</div>}
      </div>

      <div style={{ background: '#1C2030', border: '1px solid #252A3A', borderRadius: 16, padding: 28 }}>
        <h2 style={{ fontFamily: 'Lora, serif', fontSize: 18, marginBottom: 6 }}>🟢 Published ({published.length})</h2>
        <p style={{ color: '#7A8099', fontSize: 13, marginBottom: 20 }}>Live and visible to students.</p>
        {loading ? <p style={{ color: '#7A8099', fontSize: 13 }}>Loading...</p>
          : published.length === 0 ? <p style={{ color: '#7A8099', fontSize: 13 }}>No published quizzes yet.</p>
          : <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>{published.map(q => <QuizCard key={q.id} q={q} showPublish={false} />)}</div>}
      </div>
    </div>
  )
}
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!data.quiz) { setLoadError('Quiz not found.'); return }
        if (!data.questions || data.questions.length === 0) { setLoadError('This quiz has no questions yet.'); return }
        setQuiz(data.quiz)
        ...
        try {
          ...
          // still allow quiz to proceed without session tracking
        }
      } catch (e) {
        setLoadError('Failed to load quiz: ' + e.message)
      }
    }
    load()
  }, [quizId])
  ...
  if (loadError) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--cream)', fontFamily: 'Outfit, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#B85042', fontSize: 16, marginBottom: 16 }}>{loadError}</p>
        <button onClick={onExit} style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: '#0F1117', color: '#fff', cursor: 'pointer', fontSize: 14 }}>← Go Back</button>
      </div>
    </div>
  )
  if (!quiz || questions.length === 0) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--cream)', fontFamily: 'Outfit, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#7A6F67', fontSize: 15 }}>Loading quiz...</p>
      </div>
    </div>
  )
